import sys
#from paths_init import path_to_Dataset, path_to_franka_python
import rospy
...
#from real_panda.control_robot_functions import real_gripper_monitor_temperature

import rosnode
# sys.path.insert(0, path_to_franka_python() + 'real_panda')
...
class All_Controllers(object):
    ...
    def __init__(self):
        ...
        self.group = moveit_commander.MoveGroupCommander("panda_arm", ns="/")
        self.group.set_planning_time(1.5)  # was 0.3
       
        # sim gripper controller
        self.sim_gripper = moveit_commander.MoveGroupCommander("hand")
       
        # real gripper controller
        # try-except is used to avoid errors when the gripper is unplugged
        try:
            self.real_gripper = None
            # connection = create_connection(dev_name='/dev/ttyUSB0', baudrate=57600)
            # self.real_gripper = Gripper(connection, 'gripper1', [1])
            # real_gripper_monitor_temperature(self.real_gripper)
        except:
            print('real gripper is not connected')
            self.real_gripper = None
        
        # points_array (used to find the exact position of the objects wrt the base_plate)
        #self.points_array = np.load(path_to_Dataset() + 'real_and_sim/points_array_1mm_7mm.npy')

#    def real_gripper_stop_monitor_temperature(self):
#        # stop the thread real_gripper_monitor_temperature
#        if self.real_gripper:
#            self.real_gripper.stop_monitor_temperature()
...
#         scene.add_box("right_surface", p, (0.435, 1.32, 0.01))
#         # Left side of the table
#         p.pose.position.x = -0.3865
#         p.pose.position.y = 0.491
#         p.pose.position.z = 0.01
#         scene.add_box("left_surface", p, (0.435, 1.32, 0.01))
#         # Front side of the table
#         p.pose.position.x = -0.0045
#         p.pose.position.y = 0.655
#         p.pose.position.z = 0.01
#         scene.add_box("front_surface", p, (0.329, 0.99, 0.01))
#         # Rear side of the table
#         p.pose.position.x = -0.0045
#         p.pose.position.y = -0.29
#         p.pose.position.z = 0.01
#         scene.add_box("rear_surface", p, (1.20, 0.24, 0.01))
